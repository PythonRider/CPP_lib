...
int main() {
	IEEE754 a, b;
	vector<int> bin_a{0, 1, 0, 1, 0, 0}, bin_b{0, 1, 0, 1, 1, 1};

	addition_code(bin_a);
	cout << bin_to_dec(bin_a, 1) << "\t" << bin_to_dec(bin_b, 1) << "\n";
	cout << bin_to_dec(sign_bin_sum(bin_a, bin_b), 1) << "\n";
	straight_code(bin_a);
	cout << bin_to_dec(bin_a, 0) << "\t" << bin_to_dec(bin_b, 0) << "\n";
	cout << bin_to_dec(unsign_bin_sum(bin_a, bin_b), 0) << "\n";

	//addition_code(bin_a);
	a.input(-bin_to_dec(bin_a));
	a.print();
	b.input(bin_to_dec(bin_b));
	b.print();
	a = a + b;
	a.print();
	a.input(3);
